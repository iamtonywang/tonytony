...
export async function GET(req: NextRequest) {
	...
	// 최소 표시값: login_id / phone / partner_status
	const { data, error, count } = await supabase
		.from("partners")
		.select("user_id, partner_status, users!inner(login_id, phone)", { count: "exact" })
		.order("approved_at", { ascending: false })
		.range(from, to);

	if (error) {
	 return NextResponse.json({ ok: false, items: [], hasNext: false, message: "partners_fetch_failed" }, { status: 500 });
	}
