...
export async function POST(req: NextRequest) {
	...
	const paymentMemo = typeof body.paymentMemo === "string" ? body.paymentMemo : null;
	...
	const { error } = await ctx.supabase.rpc("approve_partner_settlement_request", {
		p_request_id: resolved.requestId,
		p_processed_by_admin_id: ctx.adminId,
		p_payment_memo: paymentMemo,
	});
	...
	if (error) {
		...
		if (msg.includes("request_not_approvable")) {
			return NextResponse.json({ ok: false, message: "request_not_approvable" }, { status: 400 });
		}
		return NextResponse.json({ ok: false, message: "approve_failed" }, { status: 500 });
	}
