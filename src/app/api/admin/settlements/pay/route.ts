...
export async function POST(req: NextRequest) {
	...
	const { error } = await ctx.supabase.rpc("mark_partner_settlement_request_paid", {
	...
	if (error) {
		...
		if (msg.includes("request_not_found") || msg.includes("request_has_no_items")) {
			return NextResponse.json({ ok: false, message: "not_found" }, { status: 404 });
		}
		if (
			msg.includes("request_not_payable") ||
			msg.includes("settlement_not_payable") ||
			msg.includes("settlement_not_found")
		) {
			return NextResponse.json({ ok: false, message: "pay_not_allowed" }, { status: 400 });
		}
		return NextResponse.json({ ok: false, message: "pay_failed" }, { status: 500 });
	}
